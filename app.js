...
function getSlopeContext(value) {
  ...
  // This is a terrain-context proxy from the slope raster around the road segment,
  // not a surveyed road-grade measurement.
  if (v < 3) return "low surrounding terrain slope";
  if (v < 7) return "moderate surrounding terrain slope";
  if (v < 12) return "hilly surrounding terrain";
  return "steep surrounding terrain";
}
...
function buildModeledCard(p) {
  ...
  return `
    <div class="hover-card-inner" style="border-left-color:${activeColor};">
      ...
      <b>Terrain slope proxy:</b> ${fmt(p.slp_median, 1)}°<br>\n      <span class="hover-muted">${esc(getSlopeContext(p.slp_median))}</span><br>
...
priorityControl.onAdd = function () {
  ...
  // Keep mouse, touch, and wheel interactions inside the Top 10 panel
  // from triggering map drag/zoom underneath.
  L.DomEvent.disableClickPropagation(this._div);
  L.DomEvent.disableScrollPropagation(this._div);
