...
// Dedicated panes keep the context roads visible but below modeled risk roads.
map.createPane("contextPane");
map.getPane("contextPane").style.zIndex = 410;
map.getPane("contextPane").style.pointerEvents = "auto";

map.createPane("riskPane");
map.getPane("riskPane").style.zIndex = 430;
map.getPane("riskPane").style.pointerEvents = "auto";

// Canvas renderers with tolerance make thin lines easier to hover/click.
const contextRenderer = L.canvas({ pane: "contextPane", padding: 0.6, tolerance: 12 });
const riskRenderer = L.canvas({ pane: "riskPane", padding: 0.6, tolerance: 12 });
...
titleControl.onAdd = function () {
  ...
  div.innerHTML = `
    ...
    <div class="map-title-note">
      Colored lines = modeled segments. Dashed light-gray lines = road-context segments without Phase 2 model output.
    </div>
  `;
